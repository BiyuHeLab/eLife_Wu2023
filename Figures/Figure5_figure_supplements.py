...
Plot Figure 5--figure supplement 1-3
...
for model in ["Recognition", "TwoState"]:
    ...
    for i, r in enumerate(ROIs):        
        ...
        del df_ROI, tmp
    del data, sig
del Stats_Rec, Stats_TwoState, comm_Rec, comm_TwoState, i, r, fusion, model

df_all_poststimulus = df[(df['time'] >=0)]
# %% Plot Figure 5-figure supplement 1 and 2

for n_splits in [10,20]:
       
    time_idx = np.kron(np.arange(1,n_splits+1,1), np.ones([1, int(800/n_splits)]))
    time_idx = np.append(time_idx, n_splits)
    ...
    #Wilcoxon test between models for each half respectively
        Pvals = np.zeros(n_splits,)
        Pvals[:]=np.nan
        for t, split in enumerate(np.arange(1,n_splits+1,1)):
        ...
        ax.set_xticks(np.linspace(-0.5,n_splits-0.5, n_splits+1))
        xticklabels = np.linspace(0,2,n_splits+1)
        ...
        del ax, fig, rejected, k, data, mdata, Pvals,t, xticklabels, split 
