function [beta_hat, RoiSize] = fMRI_get_roi_data(SJ, SubEVs, UserOptions, MaskName)
...
subj_dir = fullfile(ProjDir, 'ROI_masks', ['sub' SJ], RoiType);

if strcmp(RoiType, 'retinotopy') ==1
    if exist(fullfile(subj_dir,['lh' MaskName RoiSpace '.nii.gz']),'file') == 2
        RoiMasks.left = fullfile(subj_dir,['lh' MaskName RoiSpace '.nii.gz']);
    end
    if exist(fullfile(subj_dir,['rh' MaskName RoiSpace '.nii.gz']),'file') == 2
        RoiMasks.right = fullfile(subj_dir,['rh' MaskName RoiSpace '.nii.gz']);
    end
else
    if exist(fullfile(subj_dir,MaskName),'file') == 2
        RoiMasks = fullfile(subj_dir,MaskName);
    end
end
